#include <msp430.h> 

/**
 * main.c
 */
int main(void)
{
    WDTCTL = WDTPW | WDTHOLD;   // stop watchdog timer

    P1OUT &= ~BIT0;    // sets LED1 off to start
    P1OUT &= ~BIT6;    // sets LED2 off to start

    P1REN |= BIT3;    // enable pull-up/pull-down resistor on S1
    P1OUT |= BIT3;    // configure pull-up/pull-down as pull-up

    P1DIR |= BIT0;    // set Port 1.0 as output (LED1 connected to this pin)
    P1DIR |= BIT6;    // set Port 1.6 as output (LED2 connected to this pin)
    P1DIR &= ~BIT3;   // set Port 1.3 as input (Pushbutton connected to this pin)

    while (1)         // loops continuously
    {
        if (P1IN & BIT3)
        {
            P1OUT &= ~BIT0;   // turn off LED1
            P1OUT &= ~BIT6;   // turn off LED2
        }
        else
        {
            P1OUT |= BIT0;   // turn on LED1
            P1OUT |= BIT6;  // turn on LED2
        }
    }
}
